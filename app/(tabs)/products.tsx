...
export default function ProdductsScreen() {
  ...
  const handleProductInfo = (id: number) => async () => {
    try {
      const r = await GetProductById(id);
      if (r.status !== 200) {
        throw new Error("Failed to load product");
      ...
      {products.map((product: any) => (
        <View key={product.id} >
          <View style={styles.customListView}>
            ...
            <FontAwesome style={styles.pressableIcons} onPress={handleProductInfo(product.id)} name='eye' color='green' size={30}></FontAwesome>
            <FontAwesome style={styles.pressableIcons} onPress={handleUpdateProduct(product.id)} name='edit' color='blue' size={30}></FontAwesome>
            <FontAwesome style={styles.pressableIcons} onPress={handleDelete(product.id)} name='trash' color='red' size={30}></FontAwesome>
          </View>
        </View>
      ))}
